...
class DonationpointsLocation(models.Model):
    ...
    location_type_id = fields.Many2one(
        "donationpoints.location.type", 
        string=_("Type"), 
        required=True)

    owner_partner_id = fields.Many2one(
        "res.partner", 
        string=_("Owner"))  # Proprietario negozio

    owner_partner_email_id = fields.Char(
        string=_("Owner Email"), 
    ...
    owner_partner_mobile_id = fields.Char(
        string=_("Owner Mobile"), 
        related="owner_partner_id.mobile")

    contact_partner_id = fields.Many2one(
        "res.partner", 
        string=_("Contact"))  # Contatto per il negozio

    contact_partner_email_id = fields.Char(
        string=_("Contact Email"), 
    ...
    contact_partner_mobile_id = fields.Char(
        string=_("Contact Mobile"), 
        related="contact_partner_id.mobile")
